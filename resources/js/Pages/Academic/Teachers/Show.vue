...
import { ref } from 'vue';
import { 
    ArrowLeftIcon, EnvelopeIcon, PhoneIcon, IdentificationIcon, 
...
const tabs = [
    ...
    { id: 'teaching', label: 'Jadwal Mengajar' },
...
<template>
    <Head :title="`Detail Guru - ${teacher.full_name}`" />
    ...
    <AuthenticatedLayout>
        <div class="space-y-6 pb-12">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-2xl font-bold bg-gradient-to-r from-gray-800 to-gray-600 bg-clip-text text-transparent">Detail Guru</h2>
                    <div class="flex items-center gap-2 text-sm text-gray-500 mt-1">
                        <Link :href="route('yayasan.dashboard')" class="hover:text-namira-teal transition-colors">Dashboard</Link>
                        <span>/</span>
                        <Link :href="route('yayasan.teachers.index')" class="hover:text-namira-teal transition-colors">Guru</Link>
                        <span>/</span>
                        <span class="text-gray-900 font-medium">{{ teacher.full_name }}</span>
                    </div>
                </div>
                <Link :href="route('yayasan.teachers.index')" class="px-5 py-2.5 bg-white/80 backdrop-blur-sm border border-white/50 text-gray-600 rounded-2xl hover:bg-white hover:shadow-md font-bold text-sm transition-all active:scale-95 flex items-center gap-2">
            ...
            <div class="bg-white/80 backdrop-blur-xl rounded-3xl p-8 shadow-sm border border-white/50 flex flex-col md:flex-row gap-8 items-center md:items-start relative overflow-hidden">
                 <div class="absolute top-0 right-0 w-64 h-64 bg-gradient-to-bl from-amber-500/10 to-yellow-50 rounded-bl-full -mr-16 -mt-16 pointer-events-none"></div>
                 
                 <!-- Avatar -->
                 <div class="relative z-10 group">
                    <div class="w-36 h-36 rounded-full border-4 border-white shadow-xl overflow-hidden bg-gray-100 flex-shrink-0 relative">
                        <img v-if="teacher.photo" :src="`/storage/${teacher.photo}`" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110">
                 ...
                 <div class="relative z-10 flex-1 text-center md:text-left pt-2">
                    <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight mb-1">{{ teacher.full_name }}</h1>
                    <p class="text-gray-500 font-medium flex items-center justify-center md:justify-start gap-2">
                        ...
                        {{ teacher.user?.email || 'Email belum diatur' }}
                    </p>
                    ...
                    <div class="flex flex-wrap justify-center md:justify-start gap-3 mt-6">
                        ...
                        <div v-if="teacher.classroom" class="px-4 py-1.5 bg-namira-teal/10 text-namira-teal rounded-xl text-xs font-bold uppercase tracking-wide border border-namira-teal/20 flex items-center gap-1">
                            <span class="text-gray-400 font-normal normal-case mr-1">Wali Kelas:</span>
            ...
            <div class="bg-white/80 backdrop-blur-xl rounded-3xl shadow-sm border border-white/50 p-8 min-h-[400px]">
                    ...
                    <div>
                        ...
                        <dl class="space-y-6">
                            <div>
                                ...
                                <dd class="text-base font-bold text-gray-900">{{ teacher.full_name }}</dd>
                            </div>
                            <div>
                                <dt class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Nomor Induk Pegawai (NIP)</dt>
                                <dd class="text-base font-mono font-bold text-gray-700 bg-gray-50 px-3 py-1.5 rounded-lg inline-block border border-gray-200 shadow-sm">{{ teacher.nip || '-' }}</dd>
                            </div>
                            <div>
                                ...
                                <dd class="text-base font-mono font-medium">
                                     <a v-if="teacher.phone" :href="`https://wa.me/${teacher.phone}`" target="_blank" class="text-namira-teal hover:underline flex items-center gap-2 bg-teal-50 px-3 py-1.5 rounded-lg w-fit border border-teal-100">
                ...
                <div v-if="activeTab === 'teaching'" class="animate-fade-in-up">
                    <h3 class="text-lg font-bold text-gray-800 mb-6 flex items-center gap-2 pb-2 border-b border-gray-100">
                    ...
                    <div v-if="Object.keys(schedules).length === 0" class="p-8 text-center bg-gray-50 rounded-2xl border border-gray-100 border-dashed">
                        <p class="text-gray-500 font-medium">Belum ada jadwal mengajar yang diatur.</p>
                    </div>

                    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        <div v-for="(daySchedules, day) in schedules" :key="day" class="bg-gray-50 rounded-2xl p-5 border border-gray-100">
                            <h4 class="font-bold text-gray-800 mb-4 flex items-center gap-2">
                                <span class="w-2 h-6 bg-namira-teal rounded-full block"></span>
                                {{ translateDay(day) }}
                            </h4>
                            <div class="space-y-3">
                                <div v-for="sch in daySchedules" :key="sch.id" class="bg-white p-3 rounded-xl shadow-sm border border-gray-100">
                                    <div class="flex justify-between items-start mb-2">
                                        <h5 class="font-bold text-sm text-gray-800">{{ sch.subject?.name }}</h5>
                                        <span class="text-xs font-bold bg-blue-50 text-blue-600 px-2 py-0.5 rounded">{{ sch.classroom?.name }}</span>
                                    </div>
