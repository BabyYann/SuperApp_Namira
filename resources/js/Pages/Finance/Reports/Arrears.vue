...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="py-4 md:py-8 max-w-7xl mx-auto space-y-5 md:space-y-6">
            
            <!-- 📱 MOBILE PWA VIEW (block md:hidden) -->
            <div class="block md:hidden -mx-4 -mt-4 space-y-4">
                <!-- Header Gradient Card -->
                <div class="bg-gradient-to-br from-[#9f1239] to-[#0f172a] px-4 pt-5 pb-6 text-white">
                    <div class="flex items-center justify-between mb-3">
                        <div>
                            <p class="text-[10px] font-extrabold tracking-widest uppercase text-rose-300">Modul Keuangan</p>
                            <h1 class="text-xl font-black leading-tight">Laporan Tunggakan</h1>
                        </div>
                        <button 
                            @click="printRecap" 
                            class="px-3.5 py-2 bg-rose-500 hover:bg-rose-600 text-white font-extrabold text-xs rounded-xl shadow-lg flex items-center gap-1.5 active:scale-95 transition"
                        >
                            <PrinterIcon class="w-4 h-4 stroke-[2.5]" />
                    ...
                    <div class="grid grid-cols-2 gap-2 mt-4">
                        <div class="bg-white/10 backdrop-blur-sm border border-white/15 rounded-2xl p-2.5">
                            <p class="text-[9px] text-rose-200 font-bold uppercase">Total Nominal</p>
                            <p class="text-base font-black text-white leading-tight font-mono mt-0.5">{{ formatCurrency(total_arrears_sum || 0) }}</p>
                        </div>
                        <div class="bg-white/10 backdrop-blur-sm border border-white/15 rounded-2xl p-2.5">
                            <p class="text-[9px] text-rose-200 font-bold uppercase">Siswa Menunggak</p>
                            <p class="text-base font-black text-white leading-tight mt-0.5">{{ student_count || 0 }} Siswa</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="px-4 space-y-2">
                    <div class="relative">
                        ...
                        <ArrowPathIcon v-else class="w-4 h-4 absolute left-3 top-3.5 animate-spin text-rose-600" />
                        <input 
                            v-model="search" 
                            type="text" 
                            placeholder="Cari Nama Siswa atau NIS..." 
                            class="w-full pl-9 pr-3 py-2.5 bg-white border border-slate-200 rounded-xl text-xs font-semibold focus:ring-rose-500 focus:border-rose-500 shadow-sm"
                        />
                    </div>
                    <select v-model="classroom_id" class="w-full px-3 py-2 bg-white border border-slate-200 rounded-xl text-xs font-bold text-slate-700 shadow-sm">
                        <option value="">Semua Kelas</option>
                        <option v-for="cls in classrooms" :key="'mob-cls-'+cls.id" :value="cls.id">{{ cls.name }}</option>
                    </select>
                </div>

                <!-- Mobile Arrears Cards -->
                <div class="px-4 space-y-3">
                    <div v-if="students.data.length === 0" class="bg-white rounded-3xl p-8 text-center border border-slate-100 shadow-sm text-xs font-bold text-slate-400">
                        Alhamdulillah, tidak ada data siswa menunggak untuk filter ini.
                    </div>

                    <div 
                        v-for="student in students.data" 
                        :key="'mob-arr-'+student.id" 
                        class="bg-white rounded-3xl p-4 border border-slate-200 shadow-sm space-y-3 relative"
                    >
                        <div class="flex items-start justify-between gap-2">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-full bg-rose-50 text-rose-700 font-black text-xs flex items-center justify-center border border-rose-100 flex-shrink-0">
                                    {{ student.name.substring(0, 2).toUpperCase() }}
                                </div>
                                <div class="min-w-0">
                                    <h4 class="font-extrabold text-sm text-slate-900 leading-tight truncate">{{ student.name }}</h4>
                                    <p class="text-[10px] text-slate-400 font-mono mt-0.5">NIS: {{ student.nis }} • {{ student.classroom }}</p>
                                </div>
                            </div>
                        </div>

                        <div class="bg-rose-50/60 p-3 rounded-2xl border border-rose-100/70 text-xs flex items-center justify-between">
                            <span class="text-[10px] font-extrabold text-rose-700 uppercase">Total Tunggakan</span>
                            <span class="font-black text-rose-700 text-sm font-mono">{{ formatCurrency(student.total_arrears) }}</span>
                        </div>

                        <!-- Bill breakdown list -->
                        <div class="space-y-1.5 pt-1">
                            <p class="text-[10px] font-bold text-slate-400 uppercase">Rincian Tagihan ({{ student.bill_count }}):</p>
                            <div v-for="(bill, i) in student.bills" :key="'mob-b-'+i" class="flex justify-between items-center bg-slate-50 px-2.5 py-1.5 rounded-xl text-[11px] border border-slate-100">
                                <span class="font-medium text-slate-700 truncate max-w-[150px]">{{ bill.name }}</span>
                                <span class="font-bold text-slate-900 font-mono">{{ formatCurrency(bill.amount) }}</span>
                            </div>
                            <p v-if="student.bill_count > 3" class="text-[10px] text-rose-500 font-bold italic">
                        ...
                        <div class="grid grid-cols-2 gap-2 pt-2 border-t border-slate-100">
                            <a :href="student.wa_link" target="_blank" class="py-2 px-3 bg-[#25D366]/10 text-[#1da851] border border-[#25D366]/20 font-extrabold text-xs rounded-xl flex items-center justify-center gap-1.5 transition active:scale-95">
                                <ChatBubbleLeftRightIcon class="w-3.5 h-3.5" />
                                <span>Kirim WA</span>
                            </a>
                            <button @click="printLetter(student.id)" class="py-2 px-3 bg-rose-50 text-rose-700 border border-rose-200 font-extrabold text-xs rounded-xl flex items-center justify-center gap-1.5 transition active:scale-95">
