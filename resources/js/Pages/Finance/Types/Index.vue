        ...
        <div class="py-4 md:py-6 max-w-7xl mx-auto space-y-5 md:space-y-6">
            ...
            <div class="block md:hidden -mx-4 -mt-4 space-y-4">
                ...
                <div class="bg-gradient-to-br from-[#00695c] to-[#0f172a] px-4 pt-5 pb-6 text-white">
                    <div class="flex items-center justify-between mb-3">
                        <div>
                            ...
                            <h1 class="text-xl font-black leading-tight">Jenis Tagihan (Pos Bayar)</h1>
                        </div>
                        <button 
                    ...
                    <div class="bg-white/10 backdrop-blur-sm border border-white/15 rounded-2xl p-3 text-center">
                        <p class="text-2xl font-black text-white leading-none">{{ types.total || 0 }}</p>
                        <p class="text-[9px] text-teal-200 font-bold mt-1 uppercase">Pos Biaya & Tarif Default</p>
                    </div>
                </div>
                ...
                <div class="px-4 space-y-3">
                    <div v-if="types.data.length === 0" class="bg-white rounded-3xl p-8 text-center border border-slate-100 shadow-sm text-xs font-bold text-slate-400">
                        Belum ada data jenis tagihan.
                    </div>

                    <div 
                        v-for="type in types.data" 
                        :key="'mob-type-'+type.id" 
                        class="bg-white rounded-3xl p-4 border border-slate-200 shadow-sm space-y-3 relative"
                    >
                        <div class="flex items-start justify-between gap-2">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-2xl bg-purple-50 text-purple-700 font-extrabold text-xs flex items-center justify-center border border-purple-100 flex-shrink-0">
                                    {{ type.name?.substring(0,2).toUpperCase() }}
                                </div>
                                <div class="min-w-0">
                                    <h4 class="font-extrabold text-sm text-slate-900 leading-tight truncate">{{ type.name }}</h4>
                                    <span class="text-[10px] text-slate-400 font-bold uppercase block mt-0.5">{{ cycleMap[type.billing_cycle] }}</span>
                                </div>
                            </div>
                            <span class="px-2.5 py-1 text-[9px] font-black uppercase rounded-xl border flex-shrink-0"
                                  :class="type.is_active ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-rose-50 text-rose-700 border-rose-200'">
                                {{ type.is_active ? 'Aktif' : 'Non-Aktif' }}
                            </span>
                        </div>

                        <div class="bg-slate-50 p-2.5 rounded-2xl border border-slate-100 text-xs flex items-center justify-between">
                            <span class="text-[9px] text-slate-400 font-bold block uppercase">Nominal Tarif</span>
                            <span class="font-bold text-slate-800 text-xs font-mono">{{ formatCurrency(type.amount) }}</span>
                        </div>

                        <div class="pt-1 flex items-center justify-end gap-2 border-t border-slate-100">
                            <button @click="openEditModal(type)" class="px-3 py-1.5 rounded-xl text-xs font-bold text-amber-700 bg-amber-50 hover:bg-amber-100 transition flex items-center gap-1">
                                <PencilSquareIcon class="w-3.5 h-3.5" />
                                <span>Edit</span>
                            </button>
                            <button @click="confirmDelete(type)" class="px-3 py-1.5 rounded-xl text-xs font-bold text-rose-700 bg-rose-50 hover:bg-rose-100 transition flex items-center gap-1">
