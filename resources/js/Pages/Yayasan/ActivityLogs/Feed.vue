...
<template>
    ...
    <AuthenticatedLayout>
        <div class="py-4 sm:py-6 max-w-7xl mx-auto space-y-4 sm:space-y-6 px-3 sm:px-6">
            
            <!-- Page Header -->
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-3">
                <div>
                    <h2 class="font-extrabold text-xl sm:text-2xl text-gray-900 leading-tight flex items-center gap-2">
                        <SparklesIcon class="w-6 h-6 sm:w-7 sm:h-7 text-namira-teal shrink-0" />
                ...
                <Link :href="route('yayasan.dashboard')" class="inline-flex items-center gap-2 text-xs sm:text-sm font-bold text-gray-500 hover:text-namira-teal transition-colors">
                    Kembali ke Dashboard
                </Link>
            </div>

            <!-- Top Executive Summary Cards (Ultra-Responsive for PWA / Mobile) -->
            <div class="grid grid-cols-3 gap-2 sm:gap-4">
                <div class="bg-white rounded-2xl sm:rounded-3xl p-3 sm:p-5 border border-gray-150 shadow-xs flex items-center gap-2.5 sm:gap-4">
                    <div class="w-8 h-8 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-teal-50 text-teal-600 flex items-center justify-center shrink-0">
                        <ClipboardDocumentCheckIcon class="w-4 h-4 sm:w-6 sm:h-6" />
                    </div>
                    <div class="min-w-0">
                        <div class="text-[10px] sm:text-xs font-bold text-gray-400 uppercase tracking-wider truncate">Total Giat</div>
                        <div class="text-base sm:text-2xl font-black text-gray-900 leading-tight mt-0.5">
                            {{ summaryStats?.total_logs || 0 }} <span class="hidden sm:inline text-xs font-semibold text-gray-500">Kegiatan</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl sm:rounded-3xl p-3 sm:p-5 border border-gray-150 shadow-xs flex items-center gap-2.5 sm:gap-4">
                    <div class="w-8 h-8 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-indigo-50 text-indigo-600 flex items-center justify-center shrink-0">
                        <UserGroupIcon class="w-4 h-4 sm:w-6 sm:h-6" />
                    </div>
                    <div class="min-w-0">
                        <div class="text-[10px] sm:text-xs font-bold text-gray-400 uppercase tracking-wider truncate">SDM Aktif</div>
                        <div class="text-base sm:text-2xl font-black text-gray-900 leading-tight mt-0.5">
                            {{ summaryStats?.total_sdm || 0 }} <span class="hidden sm:inline text-xs font-semibold text-gray-500">Pegawai</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl sm:rounded-3xl p-3 sm:p-5 border border-gray-150 shadow-xs flex items-center gap-2.5 sm:gap-4">
                    <div class="w-8 h-8 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-amber-50 text-amber-600 flex items-center justify-center shrink-0">
                        <PhotoIcon class="w-4 h-4 sm:w-6 sm:h-6" />
                    </div>
                    <div class="min-w-0">
                        <div class="text-[10px] sm:text-xs font-bold text-gray-400 uppercase tracking-wider truncate">Bukti Foto</div>
                        <div class="text-base sm:text-2xl font-black text-gray-900 leading-tight mt-0.5">
                            {{ summaryStats?.total_photos || 0 }} <span class="hidden sm:inline text-xs font-semibold text-gray-500">Foto</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl sm:rounded-3xl border border-gray-150 p-4 sm:p-5 shadow-xs space-y-3 sm:space-y-4">
                <div class="flex flex-row items-center justify-between gap-2 border-b border-gray-100 pb-3">
                    <div class="flex items-center gap-2 text-xs font-extrabold text-gray-800 uppercase tracking-wider">
                        <FunnelIcon class="w-4 h-4 text-namira-teal shrink-0" />
                        <span class="hidden sm:inline">Filter & Pencarian Linimasa</span>
                        <span class="sm:hidden">Filter Linimasa</span>
                    </div>

                    <!-- View Mode Selector Buttons -->
                    <div class="flex items-center bg-gray-100 p-1 rounded-2xl shrink-0">
                        <button
                            @click="setViewMode('compact')"
                            :class="[
                                'px-2.5 sm:px-3 py-1.5 rounded-xl font-bold text-[11px] sm:text-xs flex items-center gap-1 transition-all cursor-pointer',
                                viewMode === 'compact' ? 'bg-white text-gray-900 shadow-xs font-extrabold' : 'text-gray-500 hover:text-gray-800'
                            ]"
                            title="Tampilan Ringkas (Tabel/Card Compact)"
                        >
                            <QueueListIcon class="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                            <span>Ringkas</span>
                        </button>
                        ...
                        >
                            <Squares2X2Icon class="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                            <span>Grid</span>
                        </button>
                        ...
                        <button
                            ...
                            title="Tampilan Feed Linimasa"
                        >
                            <Bars3Icon class="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                            <span>Feed</span>
                        </button>
                    </div>
                </div>
            ...
            <!-- Empty State -->
            <div v-if="!feedItems.data || feedItems.data.length === 0" class="bg-white rounded-2xl sm:rounded-3xl border border-dashed border-gray-200 p-12 sm:p-16 text-center space-y-3">
                <div class="w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-slate-100 text-slate-400 mx-auto flex items-center justify-center">
                    <FolderIcon class="w-7 h-7 sm:w-8 sm:h-8" />
                </div>
            ...
            <!-- VIEW MODE 1: COMPACT MODE (Desktop Table & Mobile Ultra-Sleek Cards) -->
            <div v-else-if="viewMode === 'compact'">
                <!-- Desktop Table (md and larger) -->
                <div class="hidden md:block bg-white rounded-3xl border border-gray-150 overflow-hidden shadow-xs">
                ...
                <!-- Mobile PWA Ultra-Sleek Compact Cards (md:hidden) -->
                <div class="md:hidden space-y-3">
                    <div 
                        v-for="item in feedItems.data" 
                        :key="item.id" 
                        @click="openLogDetail(item)"
                        class="bg-white rounded-2xl p-3.5 border border-gray-150 shadow-xs hover:border-namira-teal transition-all cursor-pointer space-y-2.5"
                    >
                        <!-- Top Info -->
                        <div class="flex items-center justify-between gap-2 border-b border-gray-100 pb-2">
                            <div class="flex items-center gap-2.5 min-w-0">
                                <img v-if="item.user?.profile_photo_url" :src="item.user.profile_photo_url" class="w-7 h-7 rounded-full object-cover border border-gray-100 shrink-0" />
                                <div v-else class="w-7 h-7 rounded-full bg-teal-50 border border-teal-100 flex items-center justify-center text-teal-700 font-bold text-[10px] shrink-0">
                                    {{ item.user?.name?.charAt(0) || 'P' }}
                                </div>
                                <div class="min-w-0">
                                    <div class="font-extrabold text-gray-900 text-xs truncate">{{ item.user?.name || 'Pegawai' }}</div>
                                    <div class="text-[9px] text-teal-700 font-semibold truncate">{{ item.unit?.name || '-' }}</div>
                                </div>
                            </div>
                        ...
                        <!-- Title & Time -->
                        <div>
                            <h4 class="font-extrabold text-gray-900 text-xs leading-snug line-clamp-2">{{ item.title }}</h4>
                            <div class="flex items-center justify-between text-[10px] text-gray-400 font-mono mt-1">
                                <span>{{ formatDate(item.activity_date) }} • {{ formatTime(item.activity_time) }} WIB</span>
                                <span v-if="item.photo_path" class="text-teal-600 font-bold flex items-center gap-0.5">
            ...
            <div v-else-if="viewMode === 'grid'" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-5">
                <div
                    v-for="item in feedItems.data"
                    :key="item.id"
                    class="bg-white rounded-2xl sm:rounded-3xl border border-gray-150 p-4 sm:p-5 shadow-xs hover:shadow-md transition-all flex flex-col justify-between space-y-3 sm:space-y-4"
                >
                    <div class="space-y-3">
                    ...
                    <div v-if="item.photo_path" class="pt-2">
                        <div
                            @click="openPhotoLightbox('/' + item.photo_path)"
                            class="relative h-36 sm:h-40 rounded-2xl overflow-hidden border border-gray-200 group cursor-pointer bg-slate-50 shadow-xs"
                        >
            ...
            <div v-else class="space-y-4 sm:space-y-5 max-w-4xl mx-auto">
                <div
                    v-for="item in feedItems.data"
                    :key="item.id"
                    class="bg-white rounded-2xl sm:rounded-3xl border border-gray-150 shadow-xs p-4 sm:p-6 hover:shadow-md transition-all space-y-3 sm:space-y-4"
                >
                    <!-- Header -->
                    <div class="flex items-start justify-between gap-3 border-b border-gray-100 pb-3 sm:pb-4">
                        <div class="flex items-center gap-3">
                            <div class="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-teal-50 border border-teal-100 flex items-center justify-center overflow-hidden shrink-0">
                            ...
                            <div>
                                <h4 class="font-extrabold text-gray-900 text-xs sm:text-sm">{{ item.user?.name || 'Pegawai' }}</h4>
                                <div class="flex items-center gap-2 text-[10px] sm:text-xs text-gray-400 mt-0.5">
                                    <span v-if="item.unit?.name" class="font-bold text-teal-700 bg-teal-50 px-2 py-0.5 rounded-full border border-teal-100">
                                        {{ item.unit.name }}
                                    </span>
                                    <span>•</span>
                        ...
                        <span class="px-2.5 py-1 rounded-full text-[9px] sm:text-xs font-extrabold uppercase border shrink-0" :class="categoryBadgeClass(item.category)">
                            {{ categories[item.category] || item.category }}
                        </span>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        <h3 class="font-extrabold text-gray-900 text-sm sm:text-base leading-snug">{{ item.title }}</h3>
                    ...
                    <div v-if="item.photo_path">
                        <div
                            @click="openPhotoLightbox('/' + item.photo_path)"
                            class="relative max-w-lg h-48 sm:h-64 rounded-2xl overflow-hidden border border-gray-200 group cursor-pointer bg-slate-50 shadow-xs"
                        >
                            <img :src="'/' + item.photo_path" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
        ...
        <Teleport to="body">
            <div v-if="selectedLogDetail" class="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
                <div class="bg-white rounded-3xl shadow-2xl w-full max-w-xl p-5 sm:p-6 space-y-4 max-h-[90vh] overflow-y-auto">
                    ...
                    <div class="space-y-3">
                        ...
                        <div>
                            <h4 class="text-sm sm:text-base font-extrabold text-gray-900">{{ selectedLogDetail.title }}</h4>
