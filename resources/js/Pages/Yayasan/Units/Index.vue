    ...
    <AuthenticatedLayout>
        ...
        <div class="py-4 md:py-6 max-w-7xl mx-auto space-y-5 md:space-y-6">
            ...
            <div class="block md:hidden -mx-4 -mt-4 space-y-4">
                ...
                <div class="bg-gradient-to-br from-[#009688] to-[#0f172a] px-4 pt-5 pb-6 text-white">
                    <div class="flex items-center justify-between mb-3">
                        <div>
                            ...
                            <h1 class="text-xl font-black leading-tight">Satuan Pendidikan</h1>
                        </div>
                        <Link :href="route('yayasan.units.create')">
                            <button class="px-3.5 py-2 bg-teal-500 hover:bg-teal-600 text-white font-extrabold text-xs rounded-xl shadow-lg flex items-center gap-1.5 active:scale-95 transition">
                    ...
                    <div class="grid grid-cols-2 gap-2 mt-4 text-center">
                        <div class="bg-white/10 backdrop-blur-sm border border-white/15 rounded-2xl px-3 py-2.5">
                            <p class="text-2xl font-black text-white leading-none">{{ units.length }}</p>
                            <p class="text-[10px] text-teal-200 font-bold mt-1 uppercase">Total Unit</p>
                        </div>
                        <div class="bg-white/10 backdrop-blur-sm border border-white/15 rounded-2xl px-3 py-2.5">
                            <p class="text-2xl font-black text-amber-300 leading-none">{{ filteredUnits.length }}</p>
                            <p class="text-[10px] text-amber-200 font-bold mt-1 uppercase">Unit Tampil</p>
                        </div>
                    </div>
                </div>
                ...
                <!-- Unit Mobile Touch Cards -->
                <div class="px-4 space-y-3">
                    <div v-if="filteredUnits.length === 0" class="bg-white rounded-2xl p-8 text-center border border-slate-100 shadow-sm">
                        <BuildingLibraryIcon class="w-10 h-10 mx-auto text-teal-300 mb-2" />
                        <p class="font-extrabold text-sm text-slate-800">Belum ada unit</p>
                        <p class="text-xs text-slate-400 mt-1">Unit sekolah tidak ditemukan.</p>
                    </div>
                    ...
                    >
                        <div class="flex items-center gap-3">
                            <div class="w-12 h-12 rounded-2xl flex items-center justify-center border border-slate-100 overflow-hidden bg-slate-50 shrink-0">
                                <img v-if="unit.logo" :src="`/storage/${unit.logo}`" class="w-full h-full object-cover" />
                                <div v-else class="w-full h-full bg-gradient-to-br from-teal-500 to-emerald-600 text-white flex items-center justify-center font-black text-sm">
                                    {{ unit.code ? unit.code.substring(0,2).toUpperCase() : 'UN' }}
                                </div>
                            </div>
                            <div>
                                <h3 class="font-extrabold text-slate-900 text-sm leading-tight">{{ unit.name }}</h3>
                                <p class="text-[11px] font-mono font-bold text-teal-700 mt-0.5">Kode: {{ unit.code || '-' }}</p>
                            </div>
                        </div>

                        <!-- Categories & Levels -->
                        <div class="flex flex-wrap gap-1.5 pt-1">
                            <span class="px-2.5 py-1 bg-purple-50 text-purple-700 rounded-xl text-[10px] font-black uppercase tracking-wider border border-purple-100">
                                {{ unit.category || 'School' }}
                            </span>
                            <span class="px-2.5 py-1 bg-amber-50 text-amber-700 rounded-xl text-[10px] font-black uppercase tracking-wider border border-amber-100">
                                {{ unit.level || 'General' }}
                            </span>
                        </div>
                        ...
                        <div class="pt-2 flex items-center justify-end gap-2 border-t border-slate-100">
                            <Link :href="route('yayasan.units.show', unit.id)" class="px-3 py-1.5 rounded-xl text-xs font-bold text-blue-700 bg-blue-50 hover:bg-blue-100 transition flex items-center gap-1">
                                <EyeIcon class="w-3.5 h-3.5" />
                                <span>Profil</span>
                            </Link>
                            <Link :href="route('yayasan.units.edit', unit.id)" class="px-3 py-1.5 rounded-xl text-xs font-bold text-amber-700 bg-amber-50 hover:bg-amber-100 transition flex items-center gap-1">
                                <PencilSquareIcon class="w-3.5 h-3.5" />
                                <span>Edit</span>
                            </Link>
                            <button @click="confirmDelete(unit)" class="px-3 py-1.5 rounded-xl text-xs font-bold text-red-700 bg-red-50 hover:bg-red-100 transition flex items-center gap-1">
