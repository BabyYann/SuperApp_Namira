...
<head>
    ...
    <title>419 - Sesi Kedaluwarsa | SuperApp Namira</title>
    <!-- Google Fonts -->
    ...
    <style>
        ...
        .animate-hourglass {
            animation: hourglassRotate 5s infinite ease-in-out;
            transform-origin: center;
        }
...
<body class="min-h-screen bg-[#F8FAFC] flex items-center justify-center p-6 relative overflow-hidden">
    ...
    <div class="absolute top-[-20%] right-[-10%] w-[600px] h-[600px] bg-orange-500/5 rounded-full blur-[120px] pointer-events-none"></div>
    <div class="absolute bottom-[-20%] left-[-10%] w-[500px] h-[500px] bg-amber-500/5 rounded-full blur-[100px] pointer-events-none"></div>
    ...
    <main class="relative bg-white border border-slate-100 rounded-[2.5rem] p-8 sm:p-12 max-w-xl w-full text-center shadow-[0_20px_50px_rgba(0,0,0,0.03)] transition-all duration-300">
        ...
        <div class="w-48 h-48 mx-auto mb-8 animate-float relative flex items-center justify-center">
            <svg viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-full h-full">
                ...
                <!-- Hourglass on hand -->
                <g class="animate-hourglass" transform="translate(100, 140)">
                    <rect x="-10" y="-18" width="20" height="4" fill="#D97706" rx="1" />
                    <rect x="-10" y="14" width="20" height="4" fill="#D97706" rx="1" />
                    <!-- Hourglass Glass -->
                    <path d="M-8 -14L-2 0L-8 14H8L2 0L8 -14H-8Z" fill="#FDE68A" opacity="0.8" />
                    <!-- Sand -->
                    <path d="M-6 -14L-2 0L2 0L6 -14Z" fill="#D97706" />
        ...
        <!-- Error Code -->
        <span class="inline-flex px-4 py-1.5 bg-orange-50 border border-orange-100 text-orange-700 text-xs font-bold rounded-2xl uppercase tracking-wider mb-4 shadow-sm">
            Status: 419 Page Expired
        </span>

        <!-- Description -->
        <h1 class="text-2xl font-black text-slate-800 tracking-tight leading-snug mb-3">
            Sesi Keamanan Berakhir
        </h1>
        <p class="text-slate-500 font-medium text-sm leading-relaxed max-w-md mx-auto mb-8">
            {{ $exception->getMessage() ?: 'Sesi halaman Anda telah berakhir demi alasan keamanan. Silakan muat ulang halaman ini untuk memperbarui token Anda.' }}
        </p>

        <!-- Dynamic Action Buttons -->
        <div class="flex flex-col sm:flex-row items-center justify-center gap-3">
            <button onclick="window.location.reload()" class="w-full sm:w-auto px-8 py-3 bg-orange-500 hover:bg-orange-650 text-white font-extrabold text-xs tracking-wider uppercase rounded-2xl transition-all active:scale-95 shadow-md shadow-orange-500/20">
                Muat Ulang Halaman
            </button>
        </div>
